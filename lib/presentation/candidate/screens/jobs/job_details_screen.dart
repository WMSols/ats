            ...
            children: [
              ...
              Obx(() {
                ...
                if (hasApplied) {
                  ...
                  final application = jobsController.applications
                      .firstWhereOrNull((app) => app.jobId == job.jobId);

                  if (application != null) {
                    final missingDocIds = application.requiredDocumentIds
                        .where(
                          (docId) =>
                              !application.uploadedDocumentIds.contains(docId),
                        )
                        .toList();
                    ...
                    if (missingDocIds.isNotEmpty) {
                      return AppInfoMessage(
                        message:
                            'Please upload missing documents in My Documents screen to complete your application.',
                        type: AppInfoMessageType.warning,
                        action: TextButton.icon(
                          onPressed: () {
                            Get.toNamed(AppConstants.routeCandidateDocuments);
                ...
                return Column(
                  ...
                  children: [
                    if (!hasAllDocuments && requiredDocIds.isNotEmpty)
                      AppInfoMessage(
                        message:
                            'You can apply now, but please upload required documents later in My Documents.',
                        type: AppInfoMessageType.warning,
                      ),
                    if (!hasAllDocuments && requiredDocIds.isNotEmpty)
                      AppSpacing.vertical(context, 0.02),
